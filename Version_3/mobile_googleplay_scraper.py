...
description_llm_keywords =["AI-powered", "GPT", "ChatGPT", "Claude", "Gemini", "LLaMA", "Mistral",
                           "LLM", "language model", "transformer model", "GPT-4", "GPT-3.5", "GPT-3", "DeepSeek V3", "DeepSeek R1", "GPT-4o", "Gemini Pro 1.5", "Claude"]
...
def main():
    ...
    with open('data/gs_urls_v3.txt', 'r') as file:
        for line in file:
            ...
            if app_id:
                ...
                if app_title: # Ensure app_title was successfully scraped
                    ...
                    app_entry = {
                        ...
                        "data_safety": data_safety,
                        "app_id": app_id
                        # "reviews": reviews, #to include all reviews in the json file
                        # "llm_indicator_in_review": processed_reviews_with_llm_analysis # to include all reviews with LLM analysis in the json file
    ...
    functions_with_description.save_as_json(all_apps_data, 'data/gp_data_safety_with_description_v3.json')
    print(f"Data for {app_count} apps with descriptions saved to gp_data_safety_with_description_v3.json")
...
# ____________Filter for mobile apps and output summary and save to json file____________
    ...
    for app_title, app_info in all_apps_data.items():
        ...
        if any(mobile_word in description for mobile_word in ["mobile", "phone", "phone app", "android", "smartphone"]):
            #extract llm types
            llm_apps = functions_with_description.find_llm_keywords(description, description_llm_keywords)
    ...
    with open('data/mobile_apps_summary_v3.json', 'w') as summary_file:
        json.dump(mobile_apps_summary, summary_file, indent=4)

    print(f"Saved {len(mobile_apps_summary)} mobile apps using LLMs to data/mobile_apps_summary_v3.json")
