...
def main():
    ...
    # Comparison: count vs keyword-based
    keyword_file = "data/keyword_filtered_apps.json"
    if os.path.exists(keyword_file):
        with open(keyword_file, 'r', encoding='utf-8') as f:
            keyword_data = json.load(f)
        if isinstance(keyword_data, dict):
            keyword_data = list(keyword_data.values())
        print(f"\n📊 Keyword-based AI apps: {len(keyword_data)}")
        print(f"📊 Cohere semantic-based AI apps: {len(cohere_matched_apps)}")
